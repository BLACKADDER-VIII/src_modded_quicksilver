...
// Per-rank Lamport clock and trace file. The trace file records one row per
// completed receive: "<actual_source_rank>\t<sender's SC at send time>".
...
static void trace_open(int rank)
{
    ...
    if (trace_fp) fprintf(trace_fp, "source\tclock\n");
}
...
{
    // Header is 4 ints: [0]=num_particles, [1]=padding, [2..3]=long Lamport SC.
    // 4 ints = 16 bytes keeps float_data 8-byte aligned.

    uint64_t length_int_data   = (MC_Base_Particle::num_base_ints   * buffer_size + 4) * (int)sizeof(int);
    ...
    // int_index starts at 4: [0]=num_particles, [1]=padding, [2..3]=long SC.
...
void particle_buffer_base_type::Initialize_Buffer()
{
    ...
    // int_index is 4: header is num_particles + padding + 2-int Lamport SC.
    this->int_index     = 4;
    this->float_index   = 0;
    this->char_index    = 0;
...
void particle_buffer_base_type::Reset_Offsets()
{
    ...
    uint64_t length_int_data   = (MC_Base_Particle::num_base_ints   * num_particles + 4) * (int)sizeof(int);
...
void MC_Particle_Buffer::Unpack_Particle_Buffer(int buffer_index, uint64_t &fill_vault)
{
    ...
    // Header layout: [0]=num_particles, [1]=padding, [2..3]=long Lamport SC.
    recv_buffer.num_particles = recv_buffer.int_data[int_index++];
    int_index += 3; // skip padding int + 2 ints of SC; per-particle data starts at index 4
...
void MC_Particle_Buffer::Initialize()
{
    ...
    if (mcco->processor_info->num_processors > 1)
    {
        ...
        trace_open(mcco->processor_info->rank);
    }
}
...
void MC_Particle_Buffer::Send_Particle_Buffer(int buffer)
{
    ...
    if( send_buffer.num_particles > 0 )
    {
        ...
        // Move the buffer into the extra-send list BEFORE posting the Isend so the
        // MPI_Request has a single stable address from Isend through Test/Wait.
        // std::list nodes are not relocated by insert/erase, so &it->request_list
        // stays valid for the full request lifetime. PnMPI interceptors that key
        // on the request's address now see one address per operation.
        this->task[0].extra_send_buffer.push_back(send_buffer);
        std::list<particle_buffer_base_type>::iterator it =
            --this->task[0].extra_send_buffer.end();

        // Stamp this send's Lamport SC into the pinned buffer header.
        stamp_outgoing((long*)(it->int_data + 2));

        mpiIsend(it->int_data, it->length, MPI_BYTE, it->processor,
...
void MC_Particle_Buffer::Receive_Particle_Buffers(uint64_t &fill_vault)
{
    for ( int buffer_index = 0; buffer_index < this->num_buffers; buffer_index++ )
    {
        ...
        if( flag )
        {
            // Read sender's Lamport SC out of the header before Unpack; source
            // is recv_buffer.processor since the matching Irecv pinned that rank.
            long sc = *(long*)(recv_buffer.int_data + 2);
            log_recv(recv_buffer.processor, sc);
